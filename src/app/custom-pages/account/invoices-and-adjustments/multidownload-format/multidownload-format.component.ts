...
@Component({
  selector: 'app-multidownload-format',
  templateUrl: './multidownload-format.component.html',
  styleUrls: ['./multidownload-format.component.scss']
})
export class MultidownloadFormatComponent implements OnInit {
  ...
  downloadFormat(){
   ...
   this.selectedInvoiceNo.forEach(itemm => {
    // this.getInvoiceNos.push(itemm.docNumber);
    // console.log(this.getInvoiceNos);
      this.downloadFormatService.postDownloadInvoiceFormat(itemm.docNumber,this.selectedFormat).subscribe((resp: HttpResponse<Blob>) => {
